...
from .pipeline import find_template_video, notify_complete, open_outputs, process_source, process_template
...
from .review import build_review, save_review
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        ...
        if path == "/api/media":
            try:
                target = validated_template_dir(MANAGER.output_root, query.get("work_dir", [""])[0])
                video = find_template_video(target)
            except ValueError as exc:
                self.send_json({"error": str(exc)}, HTTPStatus.BAD_REQUEST)
                return
            self.send_media(video)
            return
        if path == "/":
